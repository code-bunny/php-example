...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($errors)) {
        ...
        $contact = new Contact(['email' => $email, 'message' => $message]);
        $contact->save();
        $success = true;
    }
}
        ...
        <div class="bg-white rounded-lg p-8 flex flex-col shadow-md">
            ...
            <form method="POST" action="/contact">
                ...
                <div class="relative mb-4">
                    ...
                    <textarea id="message" name="message" class="w-full bg-white rounded border border-gray-300 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 h-32 text-base outline-none text-gray-700 py-1 px-3 resize-none leading-6 transition-colors duration-200 ease-in-out"><?= $success ? '' : htmlspecialchars($_POST['message'] ?? '') ?></textarea>
                </div>
